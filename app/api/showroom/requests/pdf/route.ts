...
import { APPROVAL_BUCKET, DEMO_REQUEST_TYPE } from '@/lib/showroom'
...
export async function GET(req: NextRequest) {
  ...
  const { data: row, error } = await sb
    .from('approval_requests').select('request_id, request_type, requester_id, pdf_url').eq('request_id', id).maybeSingle()
  if (error) {
    console.error(`[${TAG}] row lookup failed`, { id, error })
    return bad('신청을 불러오지 못했습니다.', 500)
  }
  const r = row as { request_id: number; request_type: string; requester_id: number; pdf_url: string | null } | null
  if (!r || r.request_type !== DEMO_REQUEST_TYPE) return bad('신청을 찾을 수 없습니다.', 404)
  if (r.requester_id !== caller.engineer_id && !isSuperAdmin(caller) && !canViewAdmin(caller)) return bad('Forbidden', 403)
